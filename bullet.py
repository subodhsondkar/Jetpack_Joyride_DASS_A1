					...
					if firebeam.getActivated() == 1:
						for k in range(int(screen.getScreenheight() / 4)):
							if firebeam.getShape() == 0 and int(self._base_x) + i * self._direction + 1 == firebeam.getBase_x() - k and int(self._base_y) + j == firebeam.getBase_y() + k:
								...
								dead_bullet = 1
								break
				if self._direction == 1 and int(self._base_x) + i * self._direction + 1 == enemy.getBase_x() and int(self._base_y) + j == enemy.getBase_y() or int(self._base_x) + i * self._direction + 1 == enemy.getBase_x() and int(self._base_y) + j == enemy.getBase_y() - 1 or int(self._base_x) + i * self._direction + 1 == enemy.getBase_x() - 1 and int(self._base_y) + j == enemy.getBase_y() - 1  or int(self._base_x) + i * self._direction + 1 == enemy.getBase_x() and int(self._base_y) + j == enemy.getBase_y() - 2:
					enemy.killed(player)
